<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200">{{ $ticket->subject }} — {{ $ticket->user->name }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4">
                <div class="text-sm text-gray-500 mb-1">{{ $ticket->user->name }} · {{ $ticket->created_at->format('d.m.Y H:i') }}</div>
            ...
            @if ($ticket->allowsMessages())
                <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4">
                    <form method="POST" action="{{ route('admin.tickets.messages.store', $ticket) }}">
                        @csrf
                        <textarea name="message" rows="3" class="block w-full border-gray-300 rounded-md" placeholder="{{ __('Javob yozing...') }}" required>{{ old('message') }}</textarea>
