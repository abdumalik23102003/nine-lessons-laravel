    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <form method="GET" class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4 flex flex-wrap gap-3">
                <input type="text" name="id" value="{{ request('id') }}" placeholder="{{ __('ID') }}" class="border-gray-300 rounded-md w-24">
                <input type="text" name="name" value="{{ request('name') }}" placeholder="{{ __('Ism') }}" class="border-gray-300 rounded-md flex-1 min-w-[150px]">
                <input type="text" name="email" value="{{ request('email') }}" placeholder="{{ __('Email') }}" class="border-gray-300 rounded-md flex-1 min-w-[150px]">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg overflow-hidden">
                <table class="w-full text-sm text-left">
                    ...
                    @forelse ($users as $user)
                        <tr>
                            ...
                            <td class="px-4 py-2 text-right whitespace-nowrap">
                                <a href="{{ route('admin.users.edit', $user) }}" class="text-indigo-600 text-sm">{{ __('Tahrirlash') }}</a>
                                @if (auth()->user()->isAdmin() && auth()->user()->isNot($user))
                                    <form method="POST" action="{{ route('admin.users.destroy', $user) }}" class="inline" onsubmit="return confirm('{{ __("O'chirilsinmi?") }}');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="5" class="px-4 py-6 text-center text-gray-500">{{ __('Foydalanuvchilar topilmadi.') }}</td></tr>
                    @endforelse
                    </tbody>
                </table>
            </div>
