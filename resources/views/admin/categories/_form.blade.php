...
<div class="mb-4">
    <x-input-label for="parent_id" value="Ota kategoriya" />
    <select id="parent_id" name="parent_id" class="mt-1 block w-full border-gray-300 rounded-md">
        ...
        @foreach ($tree as $node)
            <option value="{{ $node['id'] }}" @selected((int) old('parent_id', $category->parent_id) === $node['id'])>{{ $node['name'] }}</option>
        @endforeach
    </select>
...
<div class="flex items-center gap-3">
    ...
    <a href="{{ route('admin.categories.index') }}" class="text-sm text-gray-600 dark:text-gray-400">{{ __('Bekor qilish') }}</a>
</div>
