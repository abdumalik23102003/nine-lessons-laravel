            ...
            @forelse ($adverts as $advert)
                <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4 flex items-center gap-4">
                    @if ($advert->photos->isNotEmpty())
                        <img src="{{ $advert->photos->first()->getFileUrl() }}" class="h-16 w-16 object-cover rounded-md">
                    @else
                    ...
                    <div class="flex-1">
                        @if ($advert->isActive())
                            <a href="{{ route('adverts.show', $advert) }}" class="font-medium text-gray-900 dark:text-gray-100 hover:underline">{{ $advert->title }}</a>
                        @else
