<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4 text-sm text-gray-600 dark:text-gray-300 flex items-center justify-between">
            ...
            @if ($banner->reject_reason)
                <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md p-3 text-sm text-red-700 dark:text-red-300">
                    <strong>{{ __('Rad etildi') }}:</strong> {{ $banner->reject_reason }}
                </div>
            @endif

            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('cabinet.banners.update', $banner) }}" enctype="multipart/form-data">
