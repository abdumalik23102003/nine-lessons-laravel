<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200">{{ $ticket->subject }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4">
                <div class="text-sm text-gray-500 mb-1">{{ $ticket->created_at->format('d.m.Y H:i') }}</div>
                <p class="whitespace-pre-line text-gray-800 dark:text-gray-200">{{ $ticket->content }}</p>
            </div>

            @foreach ($ticket->messages as $message)
                <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4 {{ $message->user_id === $ticket->user_id ? '' : 'border-l-4 border-indigo-400' }}">
            ...
            @if ($ticket->allowsMessages())
                <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4">
                    <form method="POST" action="{{ route('cabinet.tickets.messages.store', $ticket) }}">
                        @csrf
                        <textarea name="message" rows="3" class="block w-full border-gray-300 rounded-md" placeholder="{{ __('Xabar yozing...') }}" required>{{ old('message') }}</textarea>
                        <x-input-error :messages="$errors->get('message')" class="mt-2" />
                        <x-primary-button class="mt-3">{{ __('Yuborish') }}</x-primary-button>
                    </form>

                    <form method="POST" action="{{ route('cabinet.tickets.close', $ticket) }}" class="mt-3">
                        @csrf
