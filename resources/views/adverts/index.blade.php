            ...
            <form method="GET" class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4 mb-4 flex flex-wrap gap-3">
                ...
                <select name="region_id" class="border-gray-300 rounded-md">
                    ...
                    @foreach ($regions as $region)
                        <option
                            value="{{ $region['id'] }}" @selected(request('region_id') == $region['id'])>{{ $region['name'] }}</option>
                    @endforeach
                </select>

                <input type="number" name="price_from" value="{{ request('price_from') }}"
                       placeholder="{{ __('Narx dan') }}" class="border-gray-300 rounded-md w-28">
                <input type="number" name="price_to" value="{{ request('price_to') }}"
                       placeholder="{{ __('Narx gacha') }}" class="border-gray-300 rounded-md w-28">

                <select name="sort" class="border-gray-300 rounded-md">
                    <option
                        value="" @selected(request('sort') === null || request('sort') === '')>{{ __('Saralash') }}</option>
                    <option value="newest" @selected(request('sort') === 'newest')>{{ __('Avval yangilari') }}</option>
                    <option
                        value="price_asc" @selected(request('sort') === 'price_asc')>{{ __('Narx: arzondan qimmatga') }}</option>
                    <option
                        value="price_desc" @selected(request('sort') === 'price_desc')>{{ __('Narx: qimmatdan arzonga') }}</option>
                </select>
